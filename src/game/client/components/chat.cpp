...
CChat::CChat()
{
	...
	namespace
	{
	// Link detection and colorization
	struct SUrlMatch
	{
		int m_Start;
		int m_Length;
	...
	ColorRGBA ChatLinkColor()
	{
		return color_cast<ColorRGBA, ColorHSLA>(ColorHSLA(g_Config.m_ClMessageLinkColor));
	}

	STextBoundingBox TightenLinkHoverBounds(STextBoundingBox Bounds, float FontSize)
	{
	...
	bool IsUrlBoundaryCharacter(char c)
	{
		return c == '\0' || std::isspace(static_cast<unsigned char>(c)) || c == '(' || c == '[' || c == '{' || c == '<' || c == '"' || c == '\'';
	}

	bool IsTrailingUrlPunctuation(char c)
	{
		return c == '.' || c == ',' || c == '!' || c == '?' || c == ';' || c == ':' || c == ')' || c == ']' || c == '}' || c == '"' || c == '\'';
	}

	bool IsAsciiAlphaNumOrHyphen(char c)
	{
		return std::isalnum(static_cast<unsigned char>(c)) || c == '-';
	}

	bool IsValidHostLabel(std::string_view Label)
	{
	...
	bool LinkDomainMatchesPolicy(std::string_view Host, const std::unordered_set<std::string> &vDomains)
	{
	...
	std::string NormalizeLinkDomain(std::string_view Domain)
	{
		std::string Result;
	...
	bool ExtractNormalizedHostFromClickableUrl(std::string_view Url, std::string &NormalizedHost, bool &Https, bool &Http)
	{
		Https = false;
		Http = false;
	...
	bool ExtractNormalizedHostFromBareClickableLink(std::string_view Link, std::string &NormalizedHost)
	{
	...
	bool NormalizeClickableLink(std::string_view Link, const std::unordered_set<std::string> &vSafeDomains, std::string &NormalizedUrl, std::string &NormalizedHost, bool &Https, bool &Http)
	{
	...
	std::string NormalizeLinkPolicyEntry(std::string_view Entry)
	{
	...
	void ParseLinkPolicyArray(const json_value &Value, std::unordered_set<std::string> &vDomains)
	{
		...
		{
			const json_value &Entry = Value[i];
			if(Entry.type != json_string)
				continue;

			const std::string Domain = NormalizeLinkPolicyEntry(Entry.u.string.ptr);
			if(IsValidUrlHost(Domain))
				vDomains.insert(Domain);
		}
	}

	bool TryParseClickableUrlAt(const char *pText, int Index, const std::unordered_set<std::string> &vSafeDomains, SUrlMatch &Match)
	{
	...
	std::vector<SUrlMatch> FindClickableUrlMatches(const char *pText, const std::unordered_set<std::string> &vSafeDomains)
	{
	...
	bool TryParseClickableMarkdownLinkAt(const char *pText, int Index, const std::unordered_set<std::string> &vSafeDomains, SMarkdownLinkMatch &Match)
	{
		if(pText[Index] != '[')
			return false;
	...
	std::vector<STextColorSplit> BuildUrlColorSplits(const char *pText, const std::unordered_set<std::string> &vSafeDomains)
	{
		const std::vector<SUrlMatch> vMatches = FindClickableUrlMatches(pText, vSafeDomains);
		std::vector<STextColorSplit> vColorSplits;
		vColorSplits.reserve(vMatches.size());
		for(const auto &Match : vMatches)
			vColorSplits.emplace_back(Match.m_Start, Match.m_Length, ChatLinkColor());
		return vColorSplits;
	}

	int MaxFittingUrlChars(ITextRender *pTextRender, const CTextCursor &Cursor, const char *pText, int RemainingLength, float AvailableWidth)
	{
	...
	void MeasureLinkBounds(ITextRender *pTextRender, const CTextCursor &Cursor, const char *pDisplayText, int DisplayLength, const std::string &TargetUrl, bool AlwaysConfirm, std::vector<STextBoundingBox> &vBounds, std::vector<std::string> &vLinks, std::vector<float> &vFontSizes, std::vector<bool> &vAlwaysConfirm)
	{
	...
	void TextExWithUrlColors(ITextRender *pTextRender, CTextCursor &Cursor, const char *pText, const std::unordered_set<std::string> &vSafeDomains)
	{
		int SegmentStart = 0;
		for(int i = 0; pText[i] != '\0';)
		{
			...
			if(i > SegmentStart)
				pTextRender->TextEx(&Cursor, pText + SegmentStart, i - SegmentStart);
			...
			if(!DisplayText.empty())
			{
				const auto SavedColorSplits = Cursor.m_vColorSplits;
				Cursor.m_vColorSplits.emplace_back(Cursor.m_CharCount, (int)DisplayText.size(), ChatLinkColor());
				pTextRender->TextEx(&Cursor, DisplayText.c_str(), (int)DisplayText.size());
				Cursor.m_vColorSplits = SavedColorSplits;
			}
		...
		if(pText[SegmentStart] != '\0')
			pTextRender->TextEx(&Cursor, pText + SegmentStart);
	}

	bool MouseInLinkBounds(const vec2 &MousePos, const STextBoundingBox &Bounds)
	{
		return MousePos.x >= Bounds.m_X && MousePos.x <= Bounds.Right() && MousePos.y >= Bounds.m_Y && MousePos.y <= Bounds.Bottom();
	}

	STextBoundingBox UnionLinkBounds(const STextBoundingBox &A, const STextBoundingBox &B)
	{
		const float Left = minimum(A.m_X, B.m_X);
		const float Top = minimum(A.m_Y, B.m_Y);
		const float Right = maximum(A.Right(), B.Right());
		const float Bottom = maximum(A.Bottom(), B.Bottom());
...
void CChat::ClearPendingUploadImage()
{
...
bool CChat::SetPendingUploadImage(const IInput::SClipboardImage &Image)
{
	...
	{
		ClipboardImage.Free();
		Echo("Pasted image exceeds the configured upload size limit.");
		return false;
	}
...
bool CChat::TryPasteClipboardImage()
{
	if(!g_Config.m_UcChatPasteUpload || m_PendingUploadImage.m_State == EPendingUploadState::UPLOADING)
		return false;

	IInput::SClipboardImage ClipboardImage;
	if(!Input()->GetClipboardImage(ClipboardImage))
		return false;

	if(!g_Config.m_UcChatPasteImageWarningSkip)
	{
		m_WarningPendingClipboardImage = std::move(ClipboardImage);
		GameClient()->m_Menus.PopupConfirmWithCheckbox(
			Localize("Warning"),
	...
	if(!SetPendingUploadImage(ClipboardImage))
		Echo("Unable to attach pasted image.");
	return true;
}

bool CChat::StartPendingUpload(const char *pMessagePrefix)
{
	if(!m_PendingUploadImage.HasImage())
		return false;
	if(m_PendingUploadImage.m_State == EPendingUploadState::UPLOADING)
		return true;
	if(g_Config.m_UcChatPasteUploadUrl[0] == '\0')
	{
		m_PendingUploadImage.m_State = EPendingUploadState::FAILED;
		str_copy(m_PendingUploadImage.m_aError, "Upload URL is not configured.", sizeof(m_PendingUploadImage.m_aError));
		Echo(m_PendingUploadImage.m_aError);
		return true;
	}
	...
	m_PendingUploadImage.m_Team = m_Mode == MODE_TEAM ? 1 : 0;
	...
	m_PendingUploadImage.m_State = EPendingUploadState::UPLOADING;
...
void CChat::UpdatePendingUpload()
{
	...
	if(Success)
	{
		...
		AddHistoryEntry(m_PendingUploadImage.m_Team, aLine);
...
void CChat::PendingUploadPreviewSize(float Width, float FontSize, float &PreviewW, float &PreviewH) const
{
	if(!m_PendingUploadImage.HasImage())
	{
		PreviewW = 0.0f;
...
float CChat::PendingUploadPreviewHeight(float Width, float FontSize) const
{
	if(!m_PendingUploadImage.HasImage())
		return 0.0f;
	float PreviewW = 0.0f;
...
bool CChat::PendingUploadCloseButtonRect(float X, float Y, float Width, float FontSize, SRenderRect &ButtonRect) const
{
	if(!m_PendingUploadImage.HasImage())
		return false;
...
void CChat::RenderPendingUploadPreview(float X, float Y, float Width, float Height, float FontSize)
{
	if(!m_PendingUploadImage.HasImage())
		return;
	...
	if(PendingUploadCloseButtonRect(X, Y, Width, FontSize, m_PendingUploadCloseRect))
	{
		...
		Graphics()->DrawRect(m_PendingUploadCloseRect.m_X, m_PendingUploadCloseRect.m_Y, m_PendingUploadCloseRect.m_W, m_PendingUploadCloseRect.m_H, CloseBgColor, IGraphics::CORNER_ALL, CloseRounding);

		const float LabelFontSize = maximum(8.0f, m_PendingUploadCloseRect.m_H * 0.75f);
		const float LabelWidth = TextRender()->TextWidth(LabelFontSize, "x", -1, -1);
	...
	if(Uploading || Failed)
	{
		...
		const float LabelWidth = TextRender()->TextWidth(LabelFontSize, pLabel, -1, -1);
		CTextCursor Cursor;
...
void CChat::ConfirmPasteWarning(bool DontAskAgain)
{
	...
	if(!SetPendingUploadImage(Image))
		Echo("Unable to attach pasted image.");
}

void CChat::CancelPasteWarning()
{
	m_WarningPendingClipboardImage = {};
}
...
void CChat::UpdateLinkPolicy()
{
	...
	if(m_LinkPolicyCache.m_pRequest->State() == EHttpState::DONE)
	{
		...
		if(pRoot != nullptr)
		{
			...
			{
				ParseLinkPolicyArray((*pRoot)["safe"], vSafeDomains);
				ParseLinkPolicyArray((*pRoot)["danger"], vDangerDomains);
				Parsed = true;
			}
...
bool CChat::OnInput(const IInput::CEvent &Event)
{
	...
	{
		const std::string Link = m_HoveredLink;
		HandleLinkActivation(Link, false);
		return true;
	}
	...
	else if(Event.m_Flags & IInput::FLAG_PRESS && (Event.m_Key == KEY_RETURN || Event.m_Key == KEY_KP_ENTER))
	{
		if(m_PendingUploadImage.HasImage())
		{
			StartPendingUpload(m_Input.GetString());
			m_HasSelection = false;
			m_WantsSelectionCopy = false;
			return true;
		}
	...
	else
	{
		...
		if(Event.m_Flags & IInput::FLAG_PRESS && Event.m_Key == KEY_V && Input()->ModifierIsPressed())
		{
...
void CChat::OnPrepareLines(float y, int StartLine, int HoveredTranslateLineIndex)
{
	...
	for(int i = StartLine; i < MAX_LINES; i++)
	{
		...
		if(pDisplayedTranslatedText)
		{
			...
			AppendTextWithUrlColors(TextRender(), Line.m_TextContainerIndex, LineCursor, pDisplayedTranslatedText, m_LinkPolicyCache.m_vSafeDomains, &Line.m_vLinkBounds, &Line.m_vLinks, &Line.m_vLinkFontSizes, &Line.m_vLinkAlwaysConfirm);
			if(pDisplayedTranslatedLanguage)
			{
				ColorRGBA ColorLang = Color;
				ColorLang.r *= 0.8f;
		...
		else if(pTranslatedError)
		{
			...
			Line.m_TranslateLanguageRectValid = false;
			AppendTextWithUrlColors(TextRender(), Line.m_TextContainerIndex, LineCursor, pText, m_LinkPolicyCache.m_vSafeDomains, &Line.m_vLinkBounds, &Line.m_vLinks, &Line.m_vLinkFontSizes, &Line.m_vLinkAlwaysConfirm);
			ColorRGBA ColorSub = Color;
			ColorSub.r = 0.7f;
			ColorSub.g = 0.6f;
			ColorSub.b = 0.6f;
...
void CChat::OnRender()
{
	...
	if(ChatInteractionActive)
	{
		...
		if(m_Mode != MODE_NONE)
		{
			...
			if(m_PendingUploadImage.HasImage())
			{
				PendingPreviewReserve = PendingUploadPreviewHeight(InputAreaWidth, ScaledFontSize) + maximum(6.0f, FontSize() * 0.45f);
				RenderPendingUploadPreview(x + ChatOpenOffsetX, y - PendingPreviewReserve, InputAreaWidth, Height, ScaledFontSize);
			}
		...
		const float MessageMaxWidth = maximum(40.0f, InputCursor.m_LineWidth - (InputCursor.m_X - InputCursor.m_StartX) - TranslateButtonSize * 2.0f - TranslateButtonGap * 2.0f);
		...
		if(ScrollOffset != pMouseSelection->m_Offset.y)
		{
			...
			{
				const std::vector<STextColorSplit> vInputUrlColorSplits = BuildUrlColorSplits(m_Input.GetDisplayedString(), m_LinkPolicyCache.m_vSafeDomains);
				for(const auto &Split : vInputUrlColorSplits)
					vTypingColorSplits.push_back(Split);
			}
	...
	for(int i = m_BacklogCurLine; i < MAX_LINES; i++)
	{
		...
		if(Line.m_TextContainerIndex.Valid())
		{
			...
			{
				std::vector<IGraphics::CLineItem> vUnderlineSegments;
				const size_t NumLinks = minimum(Line.m_vLinkBounds.size(), minimum(Line.m_vLinks.size(), Line.m_vLinkFontSizes.size()));
				vUnderlineSegments.reserve(NumLinks);
				int HoveredLinkIndex = -1;
				const float LinkOffsetX = ChatOpenOffsetX + BcLineXOffset;
				for(size_t LinkIndex = 0; LinkIndex < NumLinks; ++LinkIndex)
				{
					STextBoundingBox Bounds = Line.m_vLinkBounds[LinkIndex];
					...
					if(MouseInLinkBounds(MousePos, Bounds))
					{
						HoveredLinkIndex = (int)LinkIndex;
						break;
					}
				}
				...
				if(HoveredLinkIndex < 0)
				{
					for(size_t LinkIndex = 0; LinkIndex < NumLinks; ++LinkIndex)
					{
						STextBoundingBox Union = Line.m_vLinkBounds[LinkIndex];
						Union.m_X += LinkOffsetX;
						Union.MoveBy(vec2(0.0f, TextOffsetY));
						Union = TightenLinkHoverBounds(Union, Line.m_vLinkFontSizes[LinkIndex]);
						bool HasMultipleSegments = false;
						for(size_t OtherIndex = LinkIndex + 1; OtherIndex < NumLinks; ++OtherIndex)
						{
							if(Line.m_vLinks[OtherIndex] != Line.m_vLinks[LinkIndex])
								continue;
							STextBoundingBox OtherBounds = Line.m_vLinkBounds[OtherIndex];
							OtherBounds.m_X += LinkOffsetX;
							OtherBounds.MoveBy(vec2(0.0f, TextOffsetY));
							OtherBounds = TightenLinkHoverBounds(OtherBounds, Line.m_vLinkFontSizes[OtherIndex]);
							Union = UnionLinkBounds(Union, OtherBounds);
							HasMultipleSegments = true;
						}
						if(HasMultipleSegments && MouseInLinkBounds(MousePos, Union))
						{
							HoveredLinkIndex = (int)LinkIndex;
							break;
						}
				...
				if(!vUnderlineSegments.empty())
				{
					Graphics()->TextureClear();
					Graphics()->SetColor(ChatLinkColor().WithMultipliedAlpha(Blend));
					Graphics()->LinesBegin();
					Graphics()->LinesDraw(vUnderlineSegments.data(), vUnderlineSegments.size());
					Graphics()->LinesEnd();
				}
			}
