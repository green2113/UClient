...
void CMenus::PopupConfirmWithCheckbox(const char *pTitle, const char *pMessage, const char *pConfirmButtonLabel, const char *pCancelButtonLabel, const char *pCheckboxLabel, bool CheckboxValue,
	FPopupButtonCallback pfnConfirmButtonCallback, int ConfirmNextPopup, FPopupButtonCallback pfnCancelButtonCallback, int CancelNextPopup)
{
	...
	m_PopupConfirmHasCheckbox = pCheckboxLabel != nullptr && pCheckboxLabel[0] != '\0';
	m_PopupConfirmCheckboxValue = CheckboxValue;
	str_copy(m_aPopupCheckboxLabel, pCheckboxLabel ? pCheckboxLabel : "", sizeof(m_aPopupCheckboxLabel));
}

void CMenus::PopupConfirmPasteImageFromChat()
{
	GameClient()->m_Chat.ConfirmPasteWarning(m_PopupConfirmCheckboxValue);
	SetActive(false);
}

void CMenus::PopupCancelPasteImageFromChat()
{
	GameClient()->m_Chat.CancelPasteWarning();
	SetActive(false);
}
...
void CMenus::RenderPopupFullscreen(CUIRect Screen)
{
	...
	if(m_Popup == POPUP_MESSAGE || m_Popup == POPUP_CONFIRM)
	{
		...
		{
			CUIRect CheckboxRow;
			Box.HSplitBottom(58.0f, &Box, nullptr);
			Box.HSplitBottom(22.0f, &Box, &CheckboxRow);
			CheckboxRow.VMargin(100.0f, &CheckboxRow);
			if(DoButton_CheckBox(&m_PopupConfirmCheckboxValue, m_aPopupCheckboxLabel, m_PopupConfirmCheckboxValue, &CheckboxRow))
				m_PopupConfirmCheckboxValue ^= 1;
		}
