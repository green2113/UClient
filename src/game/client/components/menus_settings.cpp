...
void CMenus::RenderSettingsAppearance(CUIRect MainView)
{
	...
	else if(s_CurTab == APPEARANCE_TAB_CHAT)
	{
		...
		DoButton_CheckBoxAutoVMarginAndSet(&g_Config.m_UcChatPasteUpload, Localize("Allow clipboard image upload in chat"), &g_Config.m_UcChatPasteUpload, &LeftView, LineSize);
		DoButton_CheckBoxAutoVMarginAndSet(&g_Config.m_UcChatPasteImageWarningSkip, Localize("Skip clipboard image warning"), &g_Config.m_UcChatPasteImageWarningSkip, &LeftView, LineSize);
