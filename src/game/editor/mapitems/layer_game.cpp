...
void CLayerGame::SetTile(int x, int y, CTile Tile)
{
	if(Tile.m_Index == TILE_THROUGH_CUT && Editor()->m_SelectEntitiesImage == "DDNet")
	{
		if(!Map()->m_pFrontLayer)
		{
			...
			Editor()->m_MultiMappingSession.NotifyAddLayer(GameGroupIndex, LayerIndex, pLayerFront->m_Type, pLayerFront->m_aName, 1); // subtype 1 = front
		}
		...
		// sync the tile on the front layer explicitly вЂ” it's set directly, bypassing the normal notify path
		{
			int GameGroupIndex = std::find(Map()->m_vpGroups.begin(), Map()->m_vpGroups.end(), Map()->m_pGameGroup) - Map()->m_vpGroups.begin();
			...
			Editor()->m_MultiMappingSession.NotifyTileEdit(GameGroupIndex, FrontLayerIndex, x, y, TILE_THROUGH_CUT, 0);
			Editor()->m_MultiMappingSession.NotifyStrokeEnd();
		}
	}
	else
	{
